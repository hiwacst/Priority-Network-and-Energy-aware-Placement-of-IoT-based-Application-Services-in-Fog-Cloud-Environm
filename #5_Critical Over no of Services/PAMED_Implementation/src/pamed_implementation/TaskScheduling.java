...
public class TaskScheduling {
    ...
    public void generateTasks(int seed) {
        ...
        //JOptionPane.showMessageDialog(null, minCpuCycleOfCriticalTask+"\n"+maxCpuCycleOfCriticalTask+"\n"+minCpuCycleOfOtherTask+"\n"+maxCpuCycleOfOtherTask);
        ...
        if (noOfTasks > Variables.iot_list_c1.size()) {
            ...
            for (int i = 0; i < noOfTasks; i++) {
                ...
                * har la eshakay xomanda agar jorey aw servicay ka dyarikrawa
                ...
                String serviceType = "";
                if (mehtodNumber == 1) {
                    serviceType = listOfServiceForEachTask.get(i).get(0).toString();
...
//                else {
//                    
//                    if(indexOfCriticalServiceForOtherAlgo.contains(i))
//                    count++;
//
//                    JOptionPane.showMessageDialog(null, "ya allah    " + Variables.number_Of_Critical_Services + "\n" + Variables.NoOfService + "\n" + Variables.NoOfTask);
//                }

                Variables.task_list.add(list);
                Variables.task_list_backup.add(list);
                //  System.out.println(serviceType + " hiwa  " + Variables.task_list.get(i).get(0));
            }
    ...
    public void defineServiceForEachTasks(int seed, int noOfTasks) {
        ...
        if (this.mehtodNumber == 2 || this.mehtodNumber == 3 || this.mehtodNumber == 4 || this.mehtodNumber == 5) {   // agar random placment bw
            while (serviceIndexForEachTaskHashSet.size() < noOfTasks) {
                ...
                //  JOptionPane.showMessageDialog(null, "ggg:  "+Variables.S_cri.size());
...
//     */
    ...
    public void taskExecutionTime() {
        ...
        for (int i = 0; i < Variables.task_list.size(); i++) {
            ...
            // System.out.println(Variables.S_cri+"\n"+Variables.S_normal+"\n"+Variables.service_list);
            // System.out.println("exe:   "+ taskCPUCycle+"\t"+serviceCPU+"\t"+executionTime);
        }
...
//  ArrayList iotNodeToBrokerPath = Fog_Topology.sendToDijkestra(edges, sourceNode, 0);
                ...
                //  ArrayList brokerToFogNodePath = Fog_Topology.sendToDijkestra(edges, 0, fogOrServerIndex);
                ...
                if (delay2 <= 0) {
                    ...
                    // System.out.println(edges + "\n" + fogOrServerIndex + "\t" + brokerToFogNodePath);
                    // JOptionPane.showMessageDialog(null, brokerToFogNodePath+"\n"+edges+"\n"+fogOrServerIndex);
                }
               ...
               // System.out.println("C1: " + allDelay + "\t" + delay1 + "\t" + delay2 + "\t" + exeTime + "\t" + Variables.task_list.get(i).get(0));
...
//                double exeAndWaitedTime = addTaskExecutionAndWitedTime(i, serviceLocation);
                double allDelay = (double) delay + exeTime;
               // System.out.println("C2: " + allDelay + "\t" + delay1 + "\t" + Variables.brokerc1_to_brokerc2_delay + "\t" + delay2 + "\t" + exeTime + "\t" + Variables.task_list.get(i).get(0));
                double allDelay1 = Double.parseDouble(formatter.format(allDelay));
                Variables.task_list.get(i).add(allDelay1);
//                if (allDelay1 <= 0) {
//                    JOptionPane.showMessageDialog(null, "Delay is less than 0 "
...
//                }
               ...
               // System.out.println("S: " + allDelay + "\t" + delay1 + "\t" + delay + "\t" + exeTime + "\t" + Variables.task_list.get(i).get(0));

                //System.out.println(delay1+"\t"+delay+"\t"+exeTime);
                double allDelay1 = Double.parseDouble(formatter.format(allDelay));
                Variables.task_list.get(i).add(allDelay1);
//                if (allDelay1 <= 0) {
//                    JOptionPane.showMessageDialog(null, "Delay is less than 0 "

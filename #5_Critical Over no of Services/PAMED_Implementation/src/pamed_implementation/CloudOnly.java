...
public class CloudOnly {
    ...
    public CloudOnly(int seed) {
        cloudOnlyServicePlacement(seed);
        cloudPowerConsumption();
    }

    public void cloudOnlyServicePlacement(int seed) {
        int serviceCPU, serviceMem;
        boolean checkCloudForPlacementOfCurrentService = false;

        for (int i = 0; i < Variables.service_list.size(); i++) {

            checkCloudForPlacementOfCurrentService = checkServiceForWaitingTime(i, "cloud");

            if (checkCloudForPlacementOfCurrentService == true) {  // second cloud
...
//        } else 
//            if (location.equalsIgnoreCase("cloud")) {
            serverCPU = Integer.parseInt(Variables.cloud_server_list.get(deviceIndex).get(0).toString());
            serverMem = Integer.parseInt(Variables.cloud_server_list.get(deviceIndex).get(1).toString());
      //  }
        ...
        for (int i = 0; i < Variables.service_list.size(); i++) {
            if (i == serviceIndex) {
                ...
                serverCPU = serverCPU - serviceCPU;      // Eq-4
                serverMem = serverMem - serviceMem;     //  Eq-4
                break;
            }
        }
...
//        if (location.equalsIgnoreCase("colony1")) {
//            Variables.fog_list_c1.get(deviceIndex).set(0, serverCPU);
//            Variables.fog_list_c1.get(deviceIndex).set(1, serverMem);
...
//        } else if (location.equalsIgnoreCase("cloud")) {
            Variables.cloud_server_list.get(deviceIndex).set(0, serverCPU);
            Variables.cloud_server_list.get(deviceIndex).set(1, serverMem);
